const q4Data = {
    ...
    proscons: {
        ...
        framework: [
            ...
            { step: "5.总结建议", en: "Conclusion", detail: "关键在于如何正确地使用。" }
        ]
    }
};
...
function renderQ4(container) {
    ...
    q4Data.review.elements.forEach(el => {
        ...
        if (el.methods) {
            ...
            el.methods.forEach(m => {
                html += `
                    <div class="vocab-card" style="width:100%; text-align:left; cursor:pointer;" onclick="const d=this.querySelector('.det'); d.style.display=d.style.display==='none'?'block':'none'">
    ...
    sug.solutions.forEach(s => {
        html += `<div class="vocab-card" onclick="const d=this.querySelector('.det'); d.style.display=d.style.display==='none'?'block':'none'">
                    <span class="word" style="font-size:14px">[${s.cat}] <br> <b>${s.word}</b></span>
                    <div class="det" style="display:none"><i style="color:#d32f2f">${s.en}</i><br><small>用法：${s.usage}</small></div>
                 </div>`;
    });
    html += `</div>`;
    ...
    sug.outlines.forEach(o => {
        html += `<div class="example-box" style="background:#fffcf0; cursor:pointer;" onclick="const d=this.querySelector('.outline-det'); d.style.display=d.style.display==='none'?'block':'none'">
    ...
    // 渲染：看法类 和 利与弊类
    const others = [q4Data.opinion, q4Data.proscons];
    others.forEach(type => {
